...
def load_excel(file):
    ...
    # ✅ Load GST sheet properly (header fix)
...
def process_sheet(df):
    ...
    # 🚫 Skip small sheets (fix index error)
    if len(df) < 20:
        return None
    ...
    if party_df is not None:
        try:
            match = party_df[
                party_df["GSTIN"].astype(str).str.strip() == str(gst).strip()
            ]
            if not match.empty:
                party_name = match["Party name"].iloc[0]
        except:
            party_name = ""
